...
Mycobot机械臂与LIMO PRO通过螺钉连接在一起。LIMO PRO车顶预留了8个M3螺丝孔位用于扩展，利用其中4个正方形排布的螺丝孔位，对应Mycobot机械臂底部的4个正方形排布孔位，通过螺钉连接在一起。所对应的螺孔位置如下图所示。
...
> 注意：为了保证后续真实机械臂控制与rviz中的显示一致，机械臂与小车的安装关系应与软件包中的URDF模型文件一致。即安装时机械臂的显示屏方向应朝向小车的显示屏一侧，此时便可以通过机械臂底盘顶部的螺柱安装孔拧紧螺柱，机械臂安装示意图如下图。

<img src="./LIMO_image/1显示屏安装方向.png" 
     alt="显示屏安装" 
     width="450px"
     style="display:block; margin:0 auto;">

安装完成后，进行小车和机械臂之间的接线。机械臂的基本控制功能的实现只需要完成两根接线：机械臂供电接线与串口通信接线。

①车载计算机与机械臂之间的串口通信目前可以支持两种接线方式：方式一：通过TypeC-USB线连接车载计算机扩展接口端的USB接口与机械臂的TypeC接口；方式二：通过TypeC-TypeC线分别连接车载计算机和机械臂的TypeC接口。

②机械臂供电接线：LIMO PRO车内引出电源DC接口线与机械臂侧面的电源DC接口连接供电。

<img src="./LIMO_image/1机械臂接线.png" 
     alt="机械臂接线" 
     width="250px"
     style="display:block; margin:0 auto;">

<img src="./LIMO_image/1车载计算机接线.png" 
     alt="车载计算机接线" 
     width="250px"
     style="display:block; margin:0 auto;">
...
在使用机械臂各功能前，请先查看系统软件包的版本是否为最新版本：
